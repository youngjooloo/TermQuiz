...
$('#qnaContent').keyup(function() {
	...
	// 글자수 세기
	if (content.length == 0 || content == '') {
		$('.bdtextCount').text('0자/500자');
	} else {
		$('.bdtextCount').text(content.length + '자/500자');
	} //if
...
$('.answer_area2').keyup(function() {
	let content = $(this).val();

	// 글자수 세기
	if (content.length == 0 || content == '') {
		$('.bdtextCount2').text('0자/150자');
	} else {
		$('.bdtextCount2').text(content.length + '자/150자');
	} //if
...
function trimCheck() {
	$('#answer_area').val($('#answer_area').val().trim());
	$('#answerForm').submit();
}
